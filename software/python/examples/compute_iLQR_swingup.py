...
csv_path = os.path.join(log_dir, "ilqr_trajectory.csv")
# np.savetxt(csv_path, csv_data, delimiter=',',
#           header="time,pos,vel,torque", comments="")
save_trajectory(csv_path, data_dict)
plot_trajectory(
    time, x_trj, data_dict["des_tau"], os.path.join(log_dir, "trajectory.png"), True
)
...
# if n_x == 3:
#     ax[0].plot(
#         dt * np.linspace(0, N, np.shape(x_trj)[0]), x_trj.T[0], label="cos(theta)"
#     )
#     ax[0].plot(
#         dt * np.linspace(0, N, np.shape(x_trj)[0]), x_trj.T[1], label="sin(theta)"
#     )
# ax[0].set_ylabel("angle [rad]")
# ax[0].legend(loc="best")
# ax[1].plot(dt * np.linspace(0, N, np.shape(x_trj)[0]), x_trj.T[-1], label="theta dot")
