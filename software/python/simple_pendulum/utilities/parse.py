...
def syntax():
    parser = argparse.ArgumentParser(
        ...
        Besides the distinction in model-based controllers (gravity compensation, energy shaping, fddp, lqr, ilqr) 
        and data-driven controllers (sac, ddpg) the control methods are also divided into open and closed loop control. 
        The former execute a precomputed trajectory, which is derived from one out of two trajectory optimization 
        techniques:

            - Direct Collocation
            - Feasibility-Driven Dynamic Programming

        The latter don't rely on a precomputed trajectory, but generate their input torques online based on the 
        implemented control policy. You can choose from a variety of different control methods by adding the flag 
        of your desired controller, e.g. '-lqr' or '-energy', to 'main.py'. In order to use openloop controllers 
        you additionally need to specify the '-openloop' flag, for example to run the real pendulum with 
        Feasibility-Driven Dynamic Programming the corresponding terminal command would be 'python main.py -openloop -fddp'. 
        We further provide the option to save all measurements and plots in the results folder by adding the -save flag.
    ...
    openloop.add_argument("-fddp", action='store_true',
                          help="feasability-driven differential dynamic programming")
