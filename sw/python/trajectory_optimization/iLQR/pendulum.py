...
def pendulum_continuous_dynamics(x, u, m=0.57288, l=0.5,
    ...
    if md == sym:
        accn = (u[0] - m * g * l * md.sin(pos) - b * vel -
                coulomb_fric*md.atan(1000*vel)*2/np.pi) / inertia
    if md == np:
        accn = (u[0] - m * g * l * md.sin(pos) - b * vel -
                coulomb_fric*md.arctan(1000*vel)*2/np.pi) / inertia
    xd = np.array([vel, accn])
    return xd


def pendulum_discrete_dynamics_euler(x, u, dt, m=0.57288,
                                     l=0.5, b=0.15, cf=0.0, g=9.81):
    x_d = pendulum_continuous_dynamics(x, u, m=m, l=l, b=b, cf=cf, g=g)
    x_next = x + x_d*dt
    return x_next


def pendulum_discrete_dynamics_rungekutta(x, u, dt, m=0.57288, l=0.5,
                                          b=0.15, cf=0.0, g=9.81):
    k1 = pendulum_continuous_dynamics(x, u, m=m, l=l, b=b, cf=cf, g=g)
    k2 = pendulum_continuous_dynamics(x+0.5*dt*k1, u, m=m,
                                      l=l, b=b, cf=cf, g=g)
    k3 = pendulum_continuous_dynamics(x+0.5*dt*k2, u, m=m,
                                      l=l, b=b, cf=cf, g=g)
    k4 = pendulum_continuous_dynamics(x+dt*k3, u, m=m, l=l, b=b, cf=cf, g=g)
...
def pendulum_swingup_stage_cost(x, u, goal=[np.pi, 0], Cu=10.0, Cp=0.01,
                                Cv=0.01, Cen=0.0, m=0.57288, l=0.5, b=0.15,
                                cf=0.0, g=9.81):
    md = sym if x.dtype == object else np  # Check type for autodiff
    eps = 1e-6
...
def pendulum3_discrete_dynamics_euler(x, u, dt, m=0.57288, l=0.5, b=0.15,
                                      cf=0.0, g=9.81):
...
def pendulum3_discrete_dynamics_rungekutta(x, u, dt, m=0.57288, l=0.5,
                                           b=0.15, cf=0.0, g=9.81):
    # pendulum state x = [cos(theta), sin(theta), thetadot]
    md = sym if x.dtype == object else np  # Check type for autodiff
    if md == np:
