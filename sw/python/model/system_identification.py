...
# local imports
from model.parameters import earth
...
class SystemIdentification():
    ...
    def analyse_plant(self, vel_dict, acc_dict, tau_dict):
        g = earth.gravity
        ref_pos = meas_pos
        ...
        param_names = ["J", "m*cx", "Fv", "Fc", "offset"]   # (Jacobian (J), Viscouse Friction (Fv), Coulomb Friction (Fc)
        ...
        optres = optimize.least_squares(fun=SystemIdentification.errfunc_with_friction, x0=p0[:], args=(ref_trq, phi), max_nfev=100000, bounds=bounds)
        p1 = optres.x
        ...
        # tau = J*(qdotdot) + m*cx*(g*sin(q)) + Fv*(qdot) + Fc*(atan(100*qdot)) + offset*(1)
        print(eq)
...
if __name__ == '__main__':
    ...
    # function calls
    t, vel_dict, acc_dict, tau_dict = SystemIdentification.filter_data()
    plot.sys_id_comparison(meas_time, vel_dict, tau_dict, acc_dict)
    param_names, term_names, p1, eq, ref_trq, est_trq = SystemIdentification.analyse_plant(vel_dict, acc_dict, tau_dict)
