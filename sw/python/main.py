import sys
import os
import mini-cheetah-motor-driver-socketcan
import asyncio
import argparse
...
if __name__ == "__main__":                      
    ...
    # prepare data
    dt, des_pos, des_vel, des_tau, des_time, meas_pos, meas_vel, meas_tau, 
    meas_time, des_pos_out, des_vel_out, des_tau_in, rad2outputrev = 
    utils.prepare(data, n, gr)
    ...
    if (args.pd and args.qdd100):
        ...
        asyncio.run(control_loop.qdd100(CSV_FILE, n, dt, des_pos_out, 
                    des_vel_out, des_tau_in, meas_pos, meas_vel, meas_tau, 
                    meas_time, gr, rad2outputrev))
    
    if (args.pd and args.ak80_6):
        start, end, meas_dt, meas_pos, meas_vel, meas_tau, meas_time = 
        control_loop.ak80_6(CSV_FILE, Kp, Kd, n, dt, des_pos, des_vel, des_tau,
    ...
    # plot data
    if args.pd:
        plots.swingup(OUTPUT_FOLDER, des_pos, des_vel, des_tau, des_time, 
                      meas_pos, meas_vel, meas_tau, meas_time)
    if args.tau:
        plots.swingup(OUTPUT_FOLDER, des_pos, des_vel, des_tau, des_time, 
                      meas_pos, meas_vel, meas_tau, meas_time)
