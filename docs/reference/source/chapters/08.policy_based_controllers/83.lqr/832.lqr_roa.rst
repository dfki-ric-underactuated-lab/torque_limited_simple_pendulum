...
.. math::
    ...
    \begin{align*}\mathcal{B} = \left\{ \mathbf{x} \vert V(\mathbf{x}) < \rho \right\} = \left\{ \mathbf{x} \vert V \bar{\mathbf{x}}^{\mathrm{T}} \mathbf{S} \bar{\mathbf{x}} <\rho \right\}.\end{align*}

Where :math:`\mathbf{\bar{x}} = \mathbf{x} - \mathbf{x}^\star` denotes the deviation from the fixed point :math:`\mathbf{x}^\star`
